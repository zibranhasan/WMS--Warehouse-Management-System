...
const createZone = async (payload) => {
    ...
    if (warehouse.status !== WarehouseStatus.ACTIVE) {
        throw new AppError(httpStatus.BAD_REQUEST, "Cannot create Zone: Warehouse is inactive.");
    }
    // 2. Validate capacity
    ...
    if (existingZone) {
        throw new AppError(httpStatus.CONFLICT, `Zone code '${payload.code}' already exists in this warehouse.`);
    }
...
const getAllZones = async (query, warehouseScope) => {
    ...
    const queryBuilder = new QueryBuilder(prisma.zone, filterQuery, {
...
const updateZone = async (id, payload) => {
    ...
    if (payload.warehouseId && payload.warehouseId !== existingZone.warehouseId) {
        ...
        if (warehouse.status !== WarehouseStatus.ACTIVE) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot move Zone: Target warehouse is inactive.");
        }
        const activeAisleCount = await prisma.aisle.count({
            where: { zoneId: id, isDeleted: false },
        });
        if (activeAisleCount > 0) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot move Zone to another warehouse while active aisles exist under it.");
        }
    }
    ...
    if (payload.code || payload.warehouseId) {
        ...
        if (duplicateZone) {
            throw new AppError(httpStatus.CONFLICT, `Zone code '${targetCode}' already exists in target warehouse.`);
        }
    }
...
const updateZoneStatus = async (id, payload) => {
    const existingZone = await prisma.zone.findFirst({
        where: { id, isDeleted: false },
    });
    if (!existingZone) {
...
const deleteZone = async (id) => {
    ...
    if (activeAisleCount > 0) {
        throw new AppError(httpStatus.BAD_REQUEST, "Cannot delete zone while active aisles exist.");
    }
