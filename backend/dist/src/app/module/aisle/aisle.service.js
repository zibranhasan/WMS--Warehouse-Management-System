...
const createAisle = async (payload) => {
    ...
    if (zone.status !== LocationStatus.ACTIVE) {
        throw new AppError(httpStatus.BAD_REQUEST, "Cannot create Aisle: Parent Zone is inactive.");
    }
    // 2. Validate capacity
    ...
    if (existingAisle) {
        throw new AppError(httpStatus.CONFLICT, `Aisle code '${payload.code}' already exists in this zone.`);
    }
...
const getAllAisles = async (query, warehouseScope) => {
    ...
    const queryBuilder = new QueryBuilder(prisma.aisle, filterQuery, {
...
const getAisleShelves = async (id) => {
    const aisle = await prisma.aisle.findFirst({
        where: { id, isDeleted: false },
    });
    if (!aisle) {
...
const updateAisle = async (id, payload) => {
    const existingAisle = await prisma.aisle.findFirst({
        where: { id, isDeleted: false },
    });
    if (!existingAisle) {
    ...
    if (payload.zoneId && payload.zoneId !== existingAisle.zoneId) {
        ...
        if (zone.status !== LocationStatus.ACTIVE) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot move Aisle: Target zone is inactive.");
        }
        const activeShelfCount = await prisma.shelf.count({
            where: { aisleId: id, isDeleted: false },
        });
        if (activeShelfCount > 0) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot move Aisle to another zone while active shelves exist under it.");
        }
    }
    ...
    if (payload.code || payload.zoneId) {
        ...
        if (duplicateAisle) {
            throw new AppError(httpStatus.CONFLICT, `Aisle code '${targetCode}' already exists in target zone.`);
        }
    }
...
const updateAisleStatus = async (id, payload) => {
    const existingAisle = await prisma.aisle.findFirst({
        where: { id, isDeleted: false },
    });
    if (!existingAisle) {
...
const deleteAisle = async (id) => {
    ...
    if (activeShelfCount > 0) {
        throw new AppError(httpStatus.BAD_REQUEST, "Cannot delete aisle while active shelves exist.");
    }
