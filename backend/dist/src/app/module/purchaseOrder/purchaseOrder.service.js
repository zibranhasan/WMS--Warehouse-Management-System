...
import { NotificationType, ProductStatus, Prisma, PurchaseOrderStatus, Role, SupplierStatus, UserStatus, WarehouseStatus, } from "../../../generated/prisma/index.js";
import AppError from "../../errorHelpers/AppError";
import { prisma } from "../../lib/prisma";
import { QueryBuilder } from "../../utils/QueryBuilder";
import { InventoryService } from "../inventory/inventory.service";
import { NotificationService } from "../notification/notification.service";
import { purchaseOrderFilterableFields, purchaseOrderSearchableFields, } from "./purchaseOrder.constant";
// ---------------------------------------------------------------------------
...
const createPurchaseOrder = async (payload, userId) => {
    ...
    if (supplier.status !== SupplierStatus.ACTIVE) {
        throw new AppError(httpStatus.BAD_REQUEST, "Cannot create purchase order for an inactive supplier.");
    }
    ...
    if (warehouse.status !== WarehouseStatus.ACTIVE) {
        throw new AppError(httpStatus.BAD_REQUEST, "Cannot create purchase order for an inactive warehouse.");
    }
    // 3. Check duplicate products in item list
    const productIds = payload.items.map((i) => i.productId);
    const uniqueProductIds = new Set(productIds);
    if (uniqueProductIds.size !== productIds.length) {
        throw new AppError(httpStatus.BAD_REQUEST, "Duplicate products are not allowed in the same purchase order.");
    }
    ...
    try {
        ...
        console.error(`[Notification] Failed to query warehouse managers for PO ${newPO.id}:`, error instanceof Error ? error.message : error);
    }
    return newPO;
};
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
const getAllPurchaseOrders = async (query, warehouseScope) => {
    const queryBuilder = new QueryBuilder(prisma.purchaseOrder, query, {
...
// ---------------------------------------------------------------------------
const updatePurchaseOrder = async (id, payload, userRole) => {
    ...
    if (existingPO.status !== PurchaseOrderStatus.PENDING) {
        throw new AppError(httpStatus.BAD_REQUEST, `Cannot update Purchase Order with status '${existingPO.status}'. Only PENDING orders can be updated.`);
    }
    ...
    if (payload.supplierId && payload.supplierId !== existingPO.supplierId) {
        ...
        if (supplier.status !== SupplierStatus.ACTIVE) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot assign an inactive supplier to the purchase order.");
        }
        supplierId = payload.supplierId;
    }
    ...
    if (payload.warehouseId && payload.warehouseId !== existingPO.warehouseId) {
        ...
        if (userRole !== Role.SUPER_ADMIN && userRole !== Role.ADMIN) {
            throw new AppError(httpStatus.FORBIDDEN, "You do not have permission to change the warehouse assignment.");
        }
        const warehouse = await prisma.warehouse.findUnique({
            where: { id: payload.warehouseId },
        });
        ...
        if (warehouse.status !== WarehouseStatus.ACTIVE) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot assign an inactive warehouse to the purchase order.");
        }
        warehouseId = payload.warehouseId;
    }
    let totalAmount = existingPO.totalAmount;
    let itemDataList = null;
    if (payload.items) {
        const productIds = payload.items.map((i) => i.productId);
        const uniqueProductIds = new Set(productIds);
        if (uniqueProductIds.size !== productIds.length) {
        ...
        if (products.length !== productIds.length) {
            throw new AppError(httpStatus.NOT_FOUND, "One or more products specified in items were not found.");
        }
        for (const prod of products) {
            if (prod.status !== ProductStatus.ACTIVE) {
                throw new AppError(httpStatus.BAD_REQUEST, `Product '${prod.name}' is inactive and cannot be ordered.`);
...
const approvePurchaseOrder = async (id, userId) => {
    ...
    if (po.supplier.isDeleted || po.supplier.status !== SupplierStatus.ACTIVE) {
        throw new AppError(httpStatus.BAD_REQUEST, "Cannot approve purchase order: Supplier is inactive or deleted.");
    }
    if (po.warehouse.status !== WarehouseStatus.ACTIVE) {
        throw new AppError(httpStatus.BAD_REQUEST, "Cannot approve purchase order: Warehouse is inactive.");
    }
    for (const item of po.items) {
        if (item.product.isDeleted || item.product.status !== ProductStatus.ACTIVE) {
            throw new AppError(httpStatus.BAD_REQUEST, `Cannot approve purchase order: Product '${item.product.name}' is inactive or deleted.`);
        }
    }
    ...
    try {
        ...
        console.error(`[Notification] Failed to query creator for approved PO ${updatedPO.id}:`, error instanceof Error ? error.message : error);
    }
    return updatedPO;
};
// ---------------------------------------------------------------------------
...
const rejectPurchaseOrder = async (id, payload, userId) => {
    ...
    try {
        ...
        console.error(`[Notification] Failed to query creator for rejected PO ${updatedPO.id}:`, error instanceof Error ? error.message : error);
    }
    return updatedPO;
};
// ---------------------------------------------------------------------------
// cancelPurchaseOrder
// ---------------------------------------------------------------------------
const cancelPurchaseOrder = async (id, payload) => {
    const po = await prisma.purchaseOrder.findUnique({
        where: { id },
    });
    if (!po) {
    ...
    if (po.status !== PurchaseOrderStatus.PENDING &&
        ...
        throw new AppError(httpStatus.BAD_REQUEST, `Cannot cancel Purchase Order with status '${po.status}'. Only PENDING or APPROVED orders can be cancelled.`);
    }
    const updatedPO = await prisma.purchaseOrder.update({
        where: { id },
        data: {
...
// ---------------------------------------------------------------------------
const generateGRNNumber = async (tx) => {
    const currentYear = new Date().getFullYear();
...
const receiveGoods = async (id, payload, userId) => {
    ...
    if (initialPO.status !== PurchaseOrderStatus.APPROVED &&
    ...
    const createdReceiptId = await prisma.$transaction(async (tx) => {
        ...
        let newPOStatus = PurchaseOrderStatus.APPROVED;
        if (allFullyReceived) {
    ...
    if (finalPO) {
        try {
            ...
            console.error(`[Notification] Failed to query creator for received PO ${finalPO.id}:`, error instanceof Error ? error.message : error);
        }
    }
    return {
...
// ---------------------------------------------------------------------------
const getPurchaseOrderReceipts = async (purchaseOrderId) => {
    const po = await prisma.purchaseOrder.findUnique({
