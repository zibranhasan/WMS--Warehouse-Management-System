...
import { LocationMovementType, LocationStatus, ProductStatus, WarehouseStatus, } from "../../../generated/prisma/index.js";
import AppError from "../../errorHelpers/AppError";
import { prisma } from "../../lib/prisma";
import { QueryBuilder } from "../../utils/QueryBuilder";
import { inventoryLocationMovementFilterableFields, inventoryLocationMovementSearchableFields, inventoryLocationStockFilterableFields, inventoryLocationStockSearchableFields, } from "./inventory.constant";
/**
 * Helper function to validate complete hierarchy for a Bin within a specified Warehouse.
 */
const fetchAndValidateBinHierarchy = async (tx, binId, expectedWarehouseId) => {
    ...
    if (!shelf || shelf.isDeleted || shelf.status !== LocationStatus.ACTIVE) {
        throw new AppError(httpStatus.BAD_REQUEST, `Parent shelf for bin '${bin.code}' is inactive or deleted.`);
    }
    const { aisle } = shelf;
    if (!aisle || aisle.isDeleted || aisle.status !== LocationStatus.ACTIVE) {
        throw new AppError(httpStatus.BAD_REQUEST, `Parent aisle for bin '${bin.code}' is inactive or deleted.`);
    }
    const { zone } = aisle;
    if (!zone || zone.isDeleted || zone.status !== LocationStatus.ACTIVE) {
        throw new AppError(httpStatus.BAD_REQUEST, `Parent zone for bin '${bin.code}' is inactive or deleted.`);
    }
    if (zone.warehouseId !== expectedWarehouseId) {
        throw new AppError(httpStatus.BAD_REQUEST, `Bin '${bin.code}' does not belong to warehouse '${expectedWarehouseId}'.`);
    }
    return bin;
};
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
const allocateStock = async (payload, userId) => {
    return await prisma.$transaction(async (tx) => {
        ...
        if (warehouse.status !== WarehouseStatus.ACTIVE) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot allocate stock: Warehouse is inactive.");
        }
        // 2. Validate product
        ...
        if (product.status !== ProductStatus.ACTIVE) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot allocate stock: Product is inactive.");
        }
        // 3. Validate hierarchy & bin
        const bin = await fetchAndValidateBinHierarchy(tx, payload.binId, payload.warehouseId);
        ...
        if (payload.quantity > unallocatedStock) {
            throw new AppError(httpStatus.BAD_REQUEST, `Insufficient unallocated stock for product '${product.name}'. (Total Warehouse Stock: ${totalStock}, Already Allocated: ${totalAllocated}, Unallocated: ${unallocatedStock}, Attempted: ${payload.quantity})`);
        }
        ...
        if (binUsedCapacity + payload.quantity > bin.capacity) {
            throw new AppError(httpStatus.BAD_REQUEST, `Bin capacity exceeded. (Bin Capacity: ${bin.capacity}, Current Used: ${binUsedCapacity}, Attempted Total: ${binUsedCapacity + payload.quantity})`);
        }
...
// ---------------------------------------------------------------------------
const deallocateStock = async (payload, userId) => {
    return await prisma.$transaction(async (tx) => {
        // 1. Validate warehouse
        const warehouse = await tx.warehouse.findUnique({
            where: { id: payload.warehouseId },
        ...
        if (warehouse.status !== WarehouseStatus.ACTIVE) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot deallocate stock: Warehouse is inactive.");
        }
        // 2. Validate product
        const product = await tx.product.findFirst({
            where: { id: payload.productId, isDeleted: false },
        ...
        // 3. Validate hierarchy & bin
        await fetchAndValidateBinHierarchy(tx, payload.binId, payload.warehouseId);
        ...
        if (currentBinStock < payload.quantity) {
            throw new AppError(httpStatus.BAD_REQUEST, `Cannot deallocate: Insufficient stock in specified bin. (Current Bin Quantity: ${currentBinStock}, Attempted Deallocation: ${payload.quantity})`);
        }
...
// ---------------------------------------------------------------------------
const transferStock = async (payload, userId) => {
    return await prisma.$transaction(async (tx) => {
        // 1. Validate warehouse
        const warehouse = await tx.warehouse.findUnique({
            where: { id: payload.warehouseId },
        ...
        if (warehouse.status !== WarehouseStatus.ACTIVE) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot transfer stock: Warehouse is inactive.");
        }
        // 2. Validate product
        const product = await tx.product.findFirst({
            where: { id: payload.productId, isDeleted: false },
        ...
        if (product.status !== ProductStatus.ACTIVE) {
            throw new AppError(httpStatus.BAD_REQUEST, "Cannot transfer stock: Product is inactive.");
        }
        // 3. Validate source & destination bins hierarchy
        await fetchAndValidateBinHierarchy(tx, payload.fromBinId, payload.warehouseId);
        const toBin = await fetchAndValidateBinHierarchy(tx, payload.toBinId, payload.warehouseId);
        ...
        if (sourceCurrentStock < payload.quantity) {
            throw new AppError(httpStatus.BAD_REQUEST, `Cannot transfer: Insufficient stock in source bin. (Current Quantity: ${sourceCurrentStock}, Attempted Transfer: ${payload.quantity})`);
        }
        ...
        if (destUsedCapacity + payload.quantity > toBin.capacity) {
            throw new AppError(httpStatus.BAD_REQUEST, `Destination Bin capacity exceeded. (Bin Capacity: ${toBin.capacity}, Current Used: ${destUsedCapacity}, Attempted Transfer Total: ${destUsedCapacity + payload.quantity})`);
        }
...
// ---------------------------------------------------------------------------
const getStockByBin = async (binId) => {
...
// ---------------------------------------------------------------------------
const getProductLocations = async (productId, warehouseScope) => {
...
// ---------------------------------------------------------------------------
const getWarehouseLocationStock = async (warehouseId, query) => {
    ...
    const queryBuilder = new QueryBuilder(prisma.inventoryLocationStock, filterQuery, {
...
// ---------------------------------------------------------------------------
const getLocationMovements = async (query, warehouseScope) => {
    const where = {};
    if (warehouseScope) {
        where.warehouseId = warehouseScope;
    }
    const queryBuilder = new QueryBuilder(prisma.inventoryLocationMovement, query, {
