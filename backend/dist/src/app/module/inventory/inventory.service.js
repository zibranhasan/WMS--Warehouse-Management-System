...
import { ProductStatus, Prisma, StockMovementType, WarehouseStatus, } from "../../../generated/prisma/index.js";
import AppError from "../../errorHelpers/AppError";
import { prisma } from "../../lib/prisma";
import { QueryBuilder } from "../../utils/QueryBuilder";
import { inventoryFilterableFields, inventorySearchableFields, stockMovementFilterableFields, stockMovementSearchableFields, } from "./inventory.constant";
// ---------------------------------------------------------------------------
// getStockByWarehouse — Get all current stock for a specific warehouse
// ---------------------------------------------------------------------------
const getStockByWarehouse = async (warehouseId, query) => {
    ...
    const queryBuilder = new QueryBuilder(prisma.inventoryStock, query, {
...
// ---------------------------------------------------------------------------
const getProductStock = async (warehouseId, productId) => {
    const warehouse = await prisma.warehouse.findUnique({
        where: { id: warehouseId },
    });
    if (!warehouse) {
    ...
    if (!stock) {
        throw new AppError(httpStatus.NOT_FOUND, "No stock record found for this product in the specified warehouse.");
    }
    return {
...
// ---------------------------------------------------------------------------
const getStockMovements = async (query, warehouseScope) => {
    const where = {};
    if (warehouseScope) {
        where.warehouseId = warehouseScope;
    }
    const queryBuilder = new QueryBuilder(prisma.stockMovement, query, {
...
// ---------------------------------------------------------------------------
const getProductMovements = async (productId, query, warehouseScope) => {
    const product = await prisma.product.findFirst({
        where: { id: productId, isDeleted: false },
    });
    if (!product) {
...
// ---------------------------------------------------------------------------
const getAvailableStockTx = async (tx, warehouseId, productId) => {
...
// ---------------------------------------------------------------------------
const getInventorySummary = async (warehouseId, productId) => {
    ...
    if (allocatedStock > inventoryStock) {
        throw new AppError(httpStatus.INTERNAL_SERVER_ERROR, "Inventory allocation exceeds total warehouse stock.");
    }
