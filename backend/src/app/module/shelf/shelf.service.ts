...
import { LocationStatus, Shelf } from "../../../generated/prisma/index.js";
import AppError from "../../errorHelpers/AppError.js";
import { IQueryParams } from "../../interfaces/query.interface.js";
import { prisma } from "../../lib/prisma.js";
import { QueryBuilder } from "../../utils/QueryBuilder.js";
import { shelfFilterableFields, shelfSearchableFields } from "./shelf.constant.js";
import {
    ICreateShelf,
    IUpdateShelf,
    IUpdateShelfStatus,
} from "./shelf.interface.js";

const createShelf = async (payload: ICreateShelf) => {
    ...
    if (aisle.status !== LocationStatus.ACTIVE) {
        throw new AppError(
            ...
            "Cannot create Shelf: Parent Aisle is inactive.",
        );
    }
    ...
    if (existingShelf) {
        throw new AppError(
            httpStatus.CONFLICT,
            `Shelf code '${payload.code}' already exists in this aisle.`,
        );
    }
...
const getShelfById = async (id: string) => {
...
const getShelfBins = async (id: string) => {
    const shelf = await prisma.shelf.findFirst({
        where: { id, isDeleted: false },
    });
...
const updateShelf = async (id: string, payload: IUpdateShelf) => {
    const existingShelf = await prisma.shelf.findFirst({
        where: { id, isDeleted: false },
    });
    ...
    if (payload.aisleId && payload.aisleId !== existingShelf.aisleId) {
        ...
        if (aisle.status !== LocationStatus.ACTIVE) {
            throw new AppError(
                httpStatus.BAD_REQUEST,
                "Cannot move Shelf: Target aisle is inactive.",
            );
        }
        ...
        if (activeBinCount > 0) {
            throw new AppError(
                httpStatus.BAD_REQUEST,
                "Cannot move Shelf to another aisle while active bins exist under it.",
            );
        }
    }
    ...
    if (payload.code || payload.aisleId) {
        ...
        if (duplicateShelf) {
            throw new AppError(
                httpStatus.CONFLICT,
                `Shelf code '${targetCode}' already exists in target aisle.`,
            );
        }
    }
...
const updateShelfStatus = async (id: string, payload: IUpdateShelfStatus) => {
    const existingShelf = await prisma.shelf.findFirst({
        where: { id, isDeleted: false },
    });
...
const deleteShelf = async (id: string) => {
    ...
    if (activeBinCount > 0) {
        throw new AppError(
            httpStatus.BAD_REQUEST,
            "Cannot delete shelf while active bins exist.",
        );
    }
