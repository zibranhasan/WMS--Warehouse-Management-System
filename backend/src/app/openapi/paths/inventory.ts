...
import { ProductResponse, WarehouseResponse, UserResponse } from "../components/schemas.js";
...
export function registerInventoryPaths(registry: OpenAPIRegistry) {
    registry.registerComponent("schemas", "InventoryStock", InventoryStockResponse as never);
    registry.registerComponent("schemas", "StockMovement", StockMovementResponse as never);
    registry.registerComponent("schemas", "InventoryLocationStock", InventoryLocationStockResponse as never);
    registry.registerComponent("schemas", "LocationMovement", LocationMovementResponse as never);
    registry.registerComponent("schemas", "StockAdjustmentRequest", StockAdjustmentRequest as never);
    registry.registerComponent("schemas", "AllocateStockRequest", AllocateStockRequest as never);
    registry.registerComponent("schemas", "DeallocateStockRequest", DeallocateStockRequest as never);
    registry.registerComponent("schemas", "TransferStockRequest", TransferStockRequest as never);
    registry.registerComponent("schemas", "InventorySummaryResponse", InventorySummaryResponse as never);
    registry.registerComponent("schemas", "BinStockResponse", BinStockResponse as never);
    registry.registerComponent("schemas", "ProductLocationsResponse", ProductLocationsResponse as never);
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves a paginated list of inventory stock records for a specific warehouse. Includes warehouse and product details. Warehouse-scoped users are restricted to their assigned warehouse.",
        security: [{ cookieAuth: [] }],
        request: {
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves the full inventory summary for a product within a warehouse. Includes total stock, allocated stock, reserved stock, available stock, unallocated stock, and per-bin location breakdown sorted by zone/aisle/shelf/bin hierarchy. Warehouse-scoped users are restricted to their assigned warehouse.",
        security: [{ cookieAuth: [] }],
        request: {
            params: z.object({
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves the stock record for a specific product in a specific warehouse. Returns the current quantity. Warehouse-scoped users are restricted to their assigned warehouse.",
        security: [{ cookieAuth: [] }],
        request: {
            params: z.object({
                warehouseId: z.string().describe("Warehouse ID"),
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves a paginated audit trail of all stock movements (IN, OUT, ADJUSTMENT). Includes warehouse, product, and user who created the movement. Warehouse-scoped users see only movements for their assigned warehouse. Global roles (SUPER_ADMIN, ADMIN) see all warehouses.",
        security: [{ cookieAuth: [] }],
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves a paginated audit trail of stock movements for a specific product. Warehouse-scoped users see only movements within their assigned warehouse.",
        security: [{ cookieAuth: [] }],
        request: {
    ...
    registry.registerPath({
        ...
        description:
            "Transfers stock from one bin to another within the same warehouse. Source and destination bins must be different. Validates: warehouse and product are ACTIVE, both bins have valid ACTIVE hierarchies within the warehouse, source bin has sufficient stock, and destination bin capacity is not exceeded. Does not use FOR UPDATE row locking on inventory_location_stocks. Creates a TRANSFER audit record in InventoryLocationMovement.",
        security: [{ cookieAuth: [] }],
        request: {
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves the stock and capacity information for a specific bin. Returns the bin's full location hierarchy (zone, aisle, shelf), capacity, used capacity, available capacity, and a list of products currently stored in the bin.",
        security: [{ cookieAuth: [] }],
        request: {
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves all bin locations where a product is currently stored (quantity > 0), grouped by warehouse. Each location includes the full hierarchy (zone, aisle, shelf, bin). Warehouse-scoped users see only locations within their assigned warehouse.",
        security: [{ cookieAuth: [] }],
        request: {
            params: z.object({ productId: z.string().describe("Product ID") }),
        },
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves a paginated list of all inventory location stock records (bin-level allocations) for a warehouse. Includes product, bin, and hierarchy details. Warehouse-scoped users are restricted to their assigned warehouse.",
        security: [{ cookieAuth: [] }],
        request: {
            params: z.object({ warehouseId: z.string().describe("Warehouse ID") }),
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves a paginated audit trail of location-level stock movements (ALLOCATE, DEALLOCATE, TRANSFER). Warehouse-scoped users see only movements within their assigned warehouse.",
        security: [{ cookieAuth: [] }],
