...
export function registerPickingPaths(registry: OpenAPIRegistry) {
    registry.registerComponent("schemas", "PickingTask", PickingTaskResponse as never);
    registry.registerComponent("schemas", "PickingTaskItem", PickingTaskItemResponse as never);
    registry.registerComponent("schemas", "PickingTaskDetail", PickingTaskDetailResponse as never);
    registry.registerComponent("schemas", "PickingTaskItemDetail", PickingTaskItemDetailResponse as never);
    registry.registerComponent("schemas", "PickingAllocation", PickingAllocationResponse as never);
    registry.registerComponent("schemas", "CreatePickingTaskRequest", CreatePickingTaskRequest as never);
    registry.registerComponent("schemas", "AssignPickerRequest", AssignPickerRequest as never);
    registry.registerComponent("schemas", "PickItemsRequest", PickItemsRequest as never);
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves the picking task associated with a specific Sales Order. Returns the full picking task detail including items with allocations, location hierarchy (zone/aisle/shelf/bin), and computed remainingQuantity per item. Warehouse access is validated via checkSoParamsWarehouseAccess.",
        security: [{ cookieAuth: [] }],
        request: {
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves a single picking task with full details: warehouse, salesOrder, assignedTo, and items (with product, allocations, location hierarchy, and computed remainingQuantity). Allocations include the full zone/aisle/shelf/bin location chain. Warehouse access is validated via checkPickingWarehouseAccess.",
        security: [{ cookieAuth: [] }],
        request: {
    ...
    registry.registerPath({
        ...
        description:
            "Transitions a picking task to IN_PROGRESS. Task must not be CANCELLED or PICKED. STAFF users can only start tasks assigned to themselves (ownership restriction). SUPER_ADMIN and ADMIN can start any accessible task. Sends notification to the assigned picker. Requires SUPER_ADMIN, ADMIN, or STAFF role.",
        security: [{ cookieAuth: [] }],
        request: {
            params: z.object({ id: z.string().describe("Picking Task ID") }),
        },
