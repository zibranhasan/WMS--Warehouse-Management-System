...
import { WarehouseResponse, SupplierResponse, ProductResponse } from "../components/schemas.js";
...
export function registerPurchaseOrderPaths(registry: OpenAPIRegistry) {
    registry.registerComponent("schemas", "PurchaseOrder", PurchaseOrderResponse as never);
    registry.registerComponent("schemas", "PurchaseOrderItem", PurchaseOrderItemResponse as never);
    registry.registerComponent("schemas", "GoodsReceipt", GoodsReceiptResponse as never);
    registry.registerComponent("schemas", "GoodsReceiptItem", GoodsReceiptItemResponse as never);
    registry.registerComponent("schemas", "CreatePurchaseOrderRequest", CreatePurchaseOrderRequest as never);
    registry.registerComponent("schemas", "UpdatePurchaseOrderRequest", UpdatePurchaseOrderRequest as never);
    registry.registerComponent("schemas", "RejectPurchaseOrderRequest", RejectPurchaseOrderRequest as never);
    registry.registerComponent("schemas", "CancelPurchaseOrderRequest", CancelPurchaseOrderRequest as never);
    registry.registerComponent("schemas", "ReceiveGoodsRequest", ReceiveGoodsRequest as never);
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves a single purchase order with full details: supplier, warehouse, createdBy, approvedBy, and items (with product). Warehouse access is validated via checkPoWarehouseAccess.",
        security: [{ cookieAuth: [] }],
        request: {
    ...
    registry.registerPath({
        ...
        description:
            "Updates a PENDING purchase order. Only PENDING orders can be updated. Supplier must be ACTIVE. If items are provided, existing items are deleted and replaced (no partial updates). Products must be ACTIVE and unique within the order. Totals are recalculated server-side. Only SUPER_ADMIN or ADMIN can change the warehouse assignment. Separation of duties: only SUPER_ADMIN, ADMIN, or PROCUREMENT roles can update POs.",
        security: [{ cookieAuth: [] }],
        request: {
            params: z.object({ id: z.string().describe("Purchase Order ID") }),
    ...
    registry.registerPath({
        ...
        description:
            "Approves a PENDING purchase order. Only PENDING orders can be approved. The creator of the PO cannot approve their own order (separation of duties). Validates that the PO has items, supplier is ACTIVE, warehouse is ACTIVE, and all products are ACTIVE/deleted. Sets approvedById and approvedAt. Sends notification to the PO creator. Separation of duties: only SUPER_ADMIN, ADMIN, or WAREHOUSE_MANAGER roles can approve.",
        security: [{ cookieAuth: [] }],
        request: {
            params: z.object({ id: z.string().describe("Purchase Order ID") }),
        },
    ...
    registry.registerPath({
        ...
        description:
            "Rejects a PENDING purchase order. Only PENDING orders can be rejected. The creator of the PO cannot reject their own order (separation of duties). Sets rejectionReason if provided. Sends notification to the PO creator with the rejection reason. Separation of duties: only SUPER_ADMIN, ADMIN, or WAREHOUSE_MANAGER roles can reject.",
        security: [{ cookieAuth: [] }],
        request: {
            params: z.object({ id: z.string().describe("Purchase Order ID") }),
    ...
    registry.registerPath({
        ...
        description:
            "Cancels a PENDING or APPROVED purchase order. Only PENDING or APPROVED orders can be cancelled. Sets cancellationReason if provided. Separation of duties: only SUPER_ADMIN, ADMIN, or PROCUREMENT roles can cancel.",
        security: [{ cookieAuth: [] }],
        request: {
            params: z.object({ id: z.string().describe("Purchase Order ID") }),
    ...
    registry.registerPath({
        ...
        description:
            "Retrieves all goods receipts for a purchase order, ordered by createdAt descending. Includes receivedBy user, warehouse, and items (with product). Returns an array of GoodsReceipt objects. Warehouse access is validated via checkPoWarehouseAccess.",
        security: [{ cookieAuth: [] }],
        request: {
            params: z.object({ id: z.string().describe("Purchase Order ID") }),
        },
