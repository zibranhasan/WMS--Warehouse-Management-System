...
import { ProductResponse } from "../components/schemas.js";
...
export function registerProductPaths(registry: OpenAPIRegistry) {
    registry.registerComponent("schemas", "CreateProductRequest", CreateProductRequest as never);
    registry.registerComponent("schemas", "UpdateProductRequest", UpdateProductRequest as never);
    registry.registerComponent("schemas", "UpdateProductStatusRequest", UpdateProductStatusRequest as never);

    registry.registerPath({
        method: "post", path: "/api/v1/products", operationId: "createProduct", tags: ["Products"], summary: "Create product",
        description:
            "Creates a new product. SKU must be unique. Category must be active. Brand must be active if provided. Accepts multipart/form-data with optional image.",
        security: [{ cookieAuth: [] }],
        request: {
            body: {
                content: {
    ...
    registry.registerPath({
        ...
        description:
            "Updates product details. SKU uniqueness enforced. Accepts multipart/form-data with optional image. Old Cloudinary image deleted on replacement/removal.",
        security: [{ cookieAuth: [] }],
        request: {
