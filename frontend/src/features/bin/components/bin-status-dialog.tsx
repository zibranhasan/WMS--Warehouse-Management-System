...
import { LocationStatus, Bin } from "../bin.types";
import { LOCATION_STATUS_OPTIONS } from "../bin.schema";
import { Modal } from "@/components/shared/modal";
import { Button } from "@/components/ui/button";
import { Loader2 } from "lucide-react";

interface BinStatusDialogProps {
  bin: Bin | null;
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (binId: string, status: LocationStatus) => Promise<void>;
  isPending: boolean;
}

export function BinStatusDialog({
  bin,
  isOpen,
  onClose,
  onConfirm,
  isPending,
}: BinStatusDialogProps) {
  const [prevBinId, setPrevBinId] = useState<string | null>(bin?.id || null);
  const [selectedStatus, setSelectedStatus] = useState<LocationStatus>(
    bin?.status || "ACTIVE"
  );

  if (bin && bin.id !== prevBinId) {
    setPrevBinId(bin.id);
    setSelectedStatus(bin.status);
  }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bin) return;
    await onConfirm(bin.id, selectedStatus);
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={bin ? `Change Status: ${bin.name} (${bin.code})` : "Change Bin Status"}
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Select the physical operational status for this storage bin.
        </p>
