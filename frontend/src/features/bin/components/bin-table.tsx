...
export function BinTable({
  ...
  const columns = React.useMemo<ColumnDef<Bin>[]>(() => {
    return [
      {
        accessorKey: "code",
        header: "Code",
      ...
      },
      {
        ...
        header: "Shelf",
        cell: ({ row }) => {
          const shelf = row.original.shelf;
          return (
            <div className="flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300">
              <Grid className="h-3.5 w-3.5 text-amber-500 shrink-0" />
              <span className="font-medium">
                {shelf ? `${shelf.name} (${shelf.code})` : row.original.shelfId}
              </span>
            </div>
          );
        },
      },
      {
        ...
        header: "Aisle",
        cell: ({ row }) => {
          const aisle = row.original.shelf?.aisle;
          return (
            <div className="flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300">
      ...
      },
      {
        ...
        header: "Zone",
        cell: ({ row }) => {
          const zone = row.original.shelf?.aisle?.zone;
          return (
            <div className="flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300">
      ...
      },
      {
        ...
        header: "Warehouse",
        cell: ({ row }) => {
          const wh = row.original.shelf?.aisle?.zone?.warehouse;
          return (
            <div className="flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300">
  ...
  return (
    <DataTable
      ...
      emptyDescription="No bin records match your search query, shelf, or status filter."
    />
  );
}
