...
import { useAssignPacker } from "../packing.hooks";
...
// Props
// ---------------------------------------------------------------------------
interface AssignPackerDialogProps {
  packingTaskId: string | null;
  packingWarehouseId: string | null | undefined;
  currentPacker: { id: string; name: string; email: string } | null | undefined;
  isOpen: boolean;
          ...
          <div className="max-h-64 overflow-y-auto" role="listbox">
            {filtered.length === 0 ? (
              <p className="px-3 py-4 text-center text-xs text-slate-400">
                {isLoading ? "Loading staff..." : "No staff found for this warehouse."}
              </p>
            ) : (
              filtered.map((u) => {
...
// Component
// ---------------------------------------------------------------------------
export function AssignPackerDialog({
  packingTaskId,
  packingWarehouseId,
  currentPacker,
  isOpen,
  onOpenChange,
}: AssignPackerDialogProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      setSelectedUserId(currentPacker?.id ?? "");
    }
  }, [isOpen, currentPacker?.id]);

  // Fetch active STAFF users in the same warehouse as the packing task
  const { data: usersData, isLoading: isLoadingUsers } = useUsers({
    limit: 200,
    status: "ACTIVE",
    role: "STAFF",
    warehouseId: packingWarehouseId || undefined,
  });
  const users = usersData?.data ?? [];

  // Mutation
  const assignMutation = useAssignPacker();

  const isSubmitting = assignMutation.isPending;
  const hasChanged = selectedUserId && selectedUserId !== (currentPacker?.id ?? "");

  const handleSubmit = async () => {
    if (!packingTaskId || !selectedUserId) return;
  ...
  return (
    <Modal
      ...
      title="Assign Packer"
      description="Assign a packer to this packing task."
    ...
    >
      <div className="space-y-4 p-2">
        ...
        {/* No current packer */}
        {!currentPacker && (
          <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 px-2.5 py-2 dark:border-slate-700 dark:bg-slate-900">
            <p className="text-[11px] text-slate-500 dark:text-slate-400">
              No packer assigned yet.
            </p>
          </div>
        )}
