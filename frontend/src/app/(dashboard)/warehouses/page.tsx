...
} from "@/features/warehouse/warehouse.types";
...
type StatusFilterType = WarehouseStatus | "ALL";
...
export default function WarehousesPage() {
  ...
  const [statusTogglePendingId, setStatusTogglePendingId] = useState<
    string | null
  >(null);
  ...
  const handleCreateSubmit = async (
    ...
    await createMutation.mutateAsync(values as CreateWarehousePayload);
  ...
  const handleEditSubmit = async (
    ...
    await updateMutation.mutateAsync({
      ...
      payload: values as UpdateWarehousePayload,
    });
  ...
  const handleStatusToggle = async (
    ...
    try {
      ...
      showFeedback(
        "success",
        `Warehouse status updated to ${nextStatus}.`
      );
    } catch {
      showFeedback("error", "Failed to update warehouse status.");
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Manage physical warehouse facilities, locations, and status.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        <SearchInput
          ...
          placeholder="Search warehouses by code, name, city, or country..."
        />
      ...
      <Modal
        ...
        title={
          editingWarehouse
            ? `Edit Warehouse "${editingWarehouse.name}" (${editingWarehouse.code})`
            : "Edit Warehouse"
        }
      >
        <WarehouseForm
